            ...
            {selected && (
              <motion.section key="party" {...fade} className="space-y-6">
                ...
                <motion.div {...fade} className="flex flex-col items-center gap-3 pt-2">
                  <motion.button
                    whileHover={{
                      scale: everyoneAnswered ? 1.015 : 1,
                      boxShadow: everyoneAnswered ? '0 0 22px rgba(178,144,67,0.35)' : 'none',
                    }}
                    whileTap={{ scale: everyoneAnswered ? 0.985 : 1 }}
                    onClick={continueToSite}
                    disabled={!everyoneAnswered}
                    className={`px-6 py-3 rounded-xl text-white text-lg shadow-soft font-medium transition-all ${
                      everyoneAnswered ? 'purple-royal-gradient' : 'bg-gray-400'
                    } disabled:opacity-60`}
                  >
